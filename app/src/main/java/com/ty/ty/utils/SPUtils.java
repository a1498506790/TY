...
public class SPUtils {
     ...
     * 保存 user 对象到 sp 中
     */
    public static void setUser(Context context, String key, UserBean object){
        SharedPreferences sp = context.getSharedPreferences(getSpName(context), Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sp.edit();
        if(object != null){
            String jsonObj = GsonUtils.getIns().toJson(object);
     ...
     * 从 sp 中获取 User 对象
     */
    public static UserBean getUser(Context context, String key, String defaultValue){
        SharedPreferences sp = context.getSharedPreferences(getSpName(context), Context.MODE_PRIVATE);
        if(!TextUtils.isEmpty(sp.getString(key, defaultValue))){
            return GsonUtils.getIns().fromJson(sp.getString(key, defaultValue), UserBean.class);
        }
        return null;
    }
}
